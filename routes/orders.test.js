  ...
  describe('after login', () => {
    ...
    describe('POST /', () => {
      it('should send 200 to normal user and create order', async () => {
        ...
        const storedOrder = await models.Order.findOne().lean();
        expect(storedOrder).toMatchObject({
          items: items.map((i) => i._id),
          userId: (await models.User.findOne({ email: user0.email }).lean())
            ._id,
          total: 22,
        });
      });
      ...
      it('should send 200 to admin user and create order with repeat items', async () => {
        ...
        expect(storedOrder).toMatchObject({
          ...
          userId: (await models.User.findOne({ email: user1.email }))._id,
          total: 34,
        });
      });
    ...
    describe('GET /:id', () => {
      ...
      it('should send 200 to normal user with their order', async () => {
        ...
        expect(res.body).toMatchObject({
          ...
          userId: (
            await models.User.findOne({ email: user0.email })
          )._id.toString(),
          total: 34,
        });
      });
      ...
      it('should send 200 to admin user with their order', async () => {
        ...
        expect(res.body).toMatchObject({
          items: [item1],
          userId: (
            await models.User.findOne({ email: user1.email })
          )._id.toString(),
          total: 12,
        });
      });
